...
def confirm_mail_delete(request):
    ...
    if request.method == 'POST':
        for i in range(50):
            if request.POST.get("check" + str(i)) == "on":
                mail_address = request.POST.get("mail" + str(i))
                # print(mail_address)
                contacts_list = models.Contact.objects.filter(email=mail_address)
                for contact in contacts_list:
                    contact.valid_email = False
                    contact.save()
                error_email_addresses = models.ErrorMailAddress.objects.filter(address=mail_address)
                for error_email in error_email_addresses:
                    error_email.delete()
        return HttpResponseRedirect(reverse('crm_view_entities_list'))
    else:
        content = models.ErrorMailAddress.objects.all()
